...
## プロジェクト構成とモジュール配置
- `docker/` はアプリ本体・Poetry 設定・Dockerfile を含む作業ルートです。
...
- `poetry install` — コンテナ内またはローカル仮想環境に依存関係をインストールします。
- `poetry run pytest` — `docker/tests` 配下のユニットテストを実行します。
- `poetry run ruff check ./src` / `poetry run black ./src ./tests` — Python コードの lint/format を行います。
- `act -j build-and-push` — PR 作成前に GitHub Actions のビルドワークフローをローカルで検証します。

## コーディング規約と命名
- ターゲットは Python 3.13 で型チェックは厳格設定です。公開関数には型注釈を付与し、`poetry run mypy --pretty ./src` を通過させてください。
...
- コミット前に `poetry run pytest` を必ず実行し、メトリクス露出に影響が出る変更ではカバレッジや確認結果を PR に記載します。
